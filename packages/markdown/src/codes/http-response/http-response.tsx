...
import { MiniTitle } from "@writeme/lego";

const Circle: React.FC<{ className?: string }> = ({ className = "bg-code" }) => (
  <span aria-hidden="true" className={`inline-block h-3 w-3 rounded-full ${className}`} />
);

const HttpCircleStatus: React.FC<{ status: number }> = ({ status }) => {
...
export const HttpResponse: React.FC<Props> = ({ loading, response }) => {
  ...
  if (loading) return <span className="text-text-dim mb-2 mt-4 text-lg italic">Loading...</span>;

  if (response === null) {
    return (
      <h3 data-toc="false" className="mb-6 font-thin italic underline">
