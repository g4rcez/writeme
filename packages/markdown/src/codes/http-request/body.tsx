...
const Type = ({ children }: PropsWithChildren) => (
  <small className="text-text-slight font-thin italic">{children}</small>
);
  ...
  return (
    <section className="http-body-field">
      <label className="flex w-full items-center gap-2">
        <span className="flex cursor-text items-baseline gap-x-2">
    ...
    <section className="flex flex-col gap-y-2">
      {objectKeys.map((x) => {
        if (Array.isArray(x.value)) {
          return (
            <div className="http-body-array my-1 w-full" key={`body-section-${x.key}-${uuid.current}`}>
              <b>{x.key} </b>
              <Type>Array</Type>
